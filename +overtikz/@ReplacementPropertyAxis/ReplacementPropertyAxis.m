        ...
        function obj = ReplacementPropertyAxis(varargin)
            ...
            if ~isempty(xmatches)
                ...
                if M ~= numel(x_tix_labels)
                    numLabels = numel(x_tix_labels);
                    if M > numLabels
                        cullingStr = 'xTicks';
                        limitingDim = numLabels;
                        x_tix = x_tix(1:limitingDim);
                    else
                        cullingStr = 'xTickLabels';
                        limitingDim = M;
                        x_tix_labels(1:limitingDim) = x_tix_labels(1:limitingDim);
                    end
                    warning( ...
                        [...
                        'overtikz:' ...
                    ...
                    limitingDim = min(M, numLabels);
                    x_tix_labels_new = strings(M, 1);
                    x_tix_labels_new(1:limitingDim) = x_tix_labels(1:limitingDim);
                    set(axisHandle, tickProperty, x_tix_labels_new);
                end
            ...
            elseif ~isempty(ymatches)
                ...
                if M ~= numel(y_tix_labels)
                    numLabels = numel(y_tix_labels);
                    if M > numLabels
                        cullingStr = 'yTicks';
                        limitingDim = numLabels;
                        y_tix = y_tix(1:limitingDim);
                    else
                        cullingStr = 'yTickLabels';
                        limitingDim = M;
                        y_tix_labels(1:limitingDim) = y_tix_labels(1:limitingDim);
                    end
                    warning( ...
                        [...
                        'overtikz:' ...
